...
public class InsertController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(checkDuplicate){
                userError.setUserIDError("User ID already exists in the database!");
                validation = false;
            }
            if(fullName.length()<2){
                userError.setFullNameError("Full name is too short! Must be longer than 1 character!");
                validation = false;
            }
            if(!password.equals(confirm)){
                userError.setConfirmError("Password mismatched!");
                validation = false;
            }
            if(validation){
